...
class UserRegistrationEmailFormatterTest {

    public static final String TEST_CONTENT = "Test content";
    public static final String VERIFICATION_CODE = "000";

    @Test
    void testSubject() {
        // Given
        UserRegistrationEmailFormatter formatter = new UserRegistrationEmailFormatter(new User(), "");

        // When
        String subject = formatter.getSubject("Dr.Grey");

        // Then
        assertEquals("Dr.Grey - verificación de cuenta.", subject);
    }
    ...
    void testContent() {
        ...
        UserRegistrationEmailFormatter formatter = new UserRegistrationEmailFormatter(u, "");
        ...
        try (MockedConstruction<Context> mockedConstruction = mockConstruction(Context.class)) {
            when(templateEngine.process(eq("email/auto-registration-verification"), any(Context.class))).thenReturn(TEST_CONTENT);
